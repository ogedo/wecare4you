  ...
  return (
    <ScrollView className="flex-1 bg-white">
      <View className="pt-14 pb-6 px-6 bg-primary-500">
        ...
        <Text className="text-white text-2xl font-bold">{therapist?.user?.phone}</Text>
        {therapist?.state && (
          <Text className="text-white/80 text-sm mt-1">📍 {therapist.state}</Text>
        )}
      </View>
      ...
      <View className="px-6 py-6">
        <View className="flex-row flex-wrap gap-2 mb-4">
          {(therapist?.specializations ?? []).map((s: string) => (
            <View key={s} className="bg-primary-50 rounded-full px-3 py-1">
              <Text className="text-primary-700 text-sm">{s}</Text>
            </View>
          ))}
        </View>

        <Text className="text-neutral-700 mb-6">{therapist?.bio}</Text>

        <View className="flex-row justify-between items-center mb-6 p-4 bg-primary-50 rounded-2xl">
          <View>
            <Text className="text-primary-600 text-sm">License</Text>
            <Text className="font-semibold">{therapist?.licenseNumber}</Text>
            <Text className="text-neutral-500 text-xs">{therapist?.licenseBody}</Text>
          </View>
          <View className="items-end">
            <Text className="text-primary-600 text-sm">Session rate</Text>
            <Text className="font-bold text-xl">{formatNaira(therapist?.sessionRate ?? 0)}</Text>
          </View>
        </View>
        ...
        >
          {book.isPending ? (
            ...
            <Text className={`font-semibold text-base ${selectedSlot ? "text-white" : "text-neutral-400"}`}>
              Book & Pay {selectedSlot ? formatNaira(therapist?.sessionRate ?? 0) : ""}
            </Text>
          )}
        </TouchableOpacity>
      </View>
