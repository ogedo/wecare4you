...
export default function AdminTherapistsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin", "therapists", filter],
    queryFn: () => api.get(`/admin/therapists?status=${filter === "all" ? "" : filter}&limit=50`).then((r) => r.data),
  });
  ...
  const pendingCount = therapists.filter((t) => !t.isApproved).length;
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-neutral-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-neutral-50 border-b border-neutral-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left font-medium text-neutral-500">Licence</th>
                <th className="px-6 py-3 text-left font-medium text-neutral-500">Specialisations</th>
            ...
            <tbody className="divide-y divide-neutral-100">
              {therapists.map((t) => (
                <tr key={t.id} className="hover:bg-neutral-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {t.specializations.slice(0, 2).map((s) => (
                        <span key={s} className="bg-primary-50 text-primary-700 rounded-full px-2 py-0.5 text-xs">{s}</span>
                      ))}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{formatNaira(t.sessionRate)}</td>
                  <td className="px-6 py-4 text-neutral-500">{t.totalSessions ?? 0}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${
                      t.isApproved ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"
                    }`}>
          ...
          {therapists.length === 0 && (
            <div className="py-16 text-center text-neutral-400">No therapists found</div>
          )}
        </div>
      )}
