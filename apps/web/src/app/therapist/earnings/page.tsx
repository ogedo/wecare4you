...
export default function TherapistEarningsPage() {
  const { data, isLoading } = useQuery({
    queryKey: ["therapist", "payments"],
    queryFn: () => api.get("/admin/payouts").then((r) => r.data),
  });

  const payouts = data?.data ?? [];
  const total = payouts.reduce((s: number, p: { providerAmount?: number }) => s + (p.providerAmount ?? 0), 0);
  const pending = payouts.filter((p: { payoutSentAt?: string }) => !p.payoutSentAt);
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <div className="bg-white rounded-2xl border border-neutral-200 p-6">
          ...
          <p className="text-3xl font-bold text-neutral-900 mt-1">{formatNaira(total)}</p>
        </div>
        <div className="bg-white rounded-2xl border border-neutral-200 p-6">
